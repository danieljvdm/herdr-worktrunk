...
# runs `wt switch` with interactive hook output, then opens the checkout in a
# native Herdr workspace or a tab, according to open_mode.
...
# remote-tracking branches when show_remote_branches = true.
branch_refs=(refs/heads refs/remotes)
[[ $(worktrunk_show_remote_branches) == false ]] && branch_refs=(refs/heads)

# A matching branch switches; an unmatched query creates a new branch.
# Fall back to a plain prompt when fzf is unavailable.
if command -v fzf >/dev/null; then
  ...
  # --print-query: line 1 is the query, line 2 is the selected branch.
  query=$(sed -n 1p <<<"$choice")
  name=$(sed -n 2p <<<"$choice")
  [[ -z $name ]] && name=$query
else
